...
{
    public class MySqlErrorsMessageModule : IMessageModule
    {
        private static readonly ILog _log = LogManager.GetLogger<MySqlErrorsMessageModule>();

        private readonly ThreadLocal<ErrorStorage> _currentInbox = new ThreadLocal<ErrorStorage>();

        private readonly ErrorStorageFactory _inboxFactory;
        private IServiceBus _bus;
        private Timer _timer;

        public MySqlErrorsMessageModule(ErrorStorageFactory inboxFactory)
        {
        ...
        public void CleanupInbox(object state)
        {
            try
            {
                using (var cleanup = _inboxFactory.CreateStorage())
                {
        ...
        {
            var inbox = _inboxFactory.CreateStorage();
        ...
        private void OnMessageProcessingCompleted(CurrentMessageInformation messageInfo, Exception exception)
        {
            ...
            {
                _log.Error("Error committing ErrorStorage", ex);
                exception = ex;
            }
